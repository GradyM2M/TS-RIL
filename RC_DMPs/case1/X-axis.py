...
if __name__ == "__main__":
    ...
    # 生成泛化轨迹
    x_rollout, _, _ = dmp.rollout(x0=[9,15.5,3],xT=[10.25,15.6,3])
    x_rollout2, _, _ = dmp.rollout_ECDMP(x0=[9,15.5,3],xT=[10.25,15.6,3])
    ...
    plt.plot(x_des[:,0], 'g', linewidth=2, label='demo')
    plt.plot(x_rollout[:,0], 'b', linewidth=2, label='DMP')
    plt.plot(x_rollout2[:,0], 'r', linewidth=2, label='EC-DMP')
    ...
    plt.legend(prop = {'size':8},bbox_to_anchor=(0.3,1.0),ncols=1)
    ...
    plt.title("X-axis",fontdict = font1, fontsize=12)#标题
    ...
    plt.subplots_adjust(left=0.18, bottom=0.15, right=0.95, top=0.9,hspace=0.1,wspace=0.1)
    # plt.savefig("case1-x.png",dpi=600)
    plt.show()
