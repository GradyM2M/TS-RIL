...
if __name__ == "__main__":
    ...
    x_rollout2, dx_rollout2, ddx_rollout2 = dmp.rollout_ECDMP(x0=[9,15.5,3],xT=[10.25,15.6,3])

    # 输出用于绘图的数据，包括演示轨迹和泛化数据
    # np.savetxt(r'D:\大论文\实验代码\Joing_dmps\numerical_test\case1\x_des', x_des)
    # np.savetxt(r'D:\大论文\实验代码\Joing_dmps\numerical_test\case1\dmp(v)', dx_rollout)
    # np.savetxt(r'D:\大论文\实验代码\Joing_dmps\numerical_test\case1\dmp(a)', ddx_rollout)
    # np.savetxt(r'D:\大论文\实验代码\Joing_dmps\numerical_test\case1\tra_x', tra_x.T)
    # np.savetxt(r'D:\大论文\实验代码\Joing_dmps\numerical_test\case1\tra_y', tra_y.T)
    # np.savetxt(r'D:\大论文\实验代码\experiment3\case1\ec-dmp(p)', x_rollout2)
    # np.savetxt(r'D:\大论文\实验代码\Joing_dmps\numerical_test\case1\ec-dmp(v)', dx_rollout2)
    # np.savetxt(r'D:\大论文\实验代码\Joing_dmps\numerical_test\case1\ec-dmp(a)', ddx_rollout2)
    ...
    plt.plot(x_des[:,1], 'g', linewidth=2, label='reference')
    plt.plot(x_rollout[:,1], 'b', linewidth=2, label='DMP')
    plt.plot(x_rollout2[:,1], 'r', linewidth=2, label='EC-DMP')
    plt.plot(np.ones(len(x_des[:,1]))*18, 'black', linestyle=':', linewidth=2, label='Safe Limits')
    plt.plot(np.ones(len(x_des[:,1]))*20, 'black', linewidth=2, label='Safe Margins')
    plt.fill_between(np.linspace(0,3000,3000),np.ones(3000)*18,np.ones(3000)*20,facecolor = 'yellow', alpha = 0.3)

    print(20-max(x_rollout2[:,1]))
