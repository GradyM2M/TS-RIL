...
if __name__ == "__main__":
    ...
    plt.plot(np.ones(len(x_des[:,1]))*2, 'black', linestyle=':', linewidth=2)
    plt.plot(np.ones(len(x_des[:,1]))*0, 'black', linewidth=2, )
    plt.fill_between(np.linspace(0,3000,3000),np.ones(3000)*0,np.ones(3000)*2,facecolor = 'yellow', alpha = 0.3)

    plt.legend(prop = {'size':8},bbox_to_anchor=(0.5,0.4),ncols=1)
    ...
    plt.savefig("case2-Y.png",dpi=600)
    plt.show()
