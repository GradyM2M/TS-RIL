...
class CartesianDMP():
    ...
    def imitate(self, pose_demo, sampling_rate=100, oversampling=True):
        ...
        if not oversampling:
            ...
            self.N = 3*pose_demo.shape[0] # 10-fold oversample
            print(self.N) 
            self.dt = self.T / self.N
...
if __name__ == "__main__":
    ...
    plt.plot(x_des[:,2], 'g', linewidth=2, label='reference')
    plt.plot(x_rollout[:,2], 'b', linewidth=2, label='DMP')
    plt.plot(x_rollout2[:,2], 'r', linewidth=2, label='EC-DMP')
    ...
    plt.legend(prop = {'size':8},bbox_to_anchor=(0.45,0.4))
    ...
    plt.savefig("case3-z.png",dpi=600)
    plt.show()
