...
if __name__ == "__main__":
    ...
    rot = np.ones((2,2))
    print(rot)
    rot[0] = [np.cos(-3*np.pi/4),-np.sin(-3*np.pi/4)]
    rot[1] = [np.sin(-3*np.pi/4),np.cos(-3*np.pi/4)]
    ...
    print(x_des[0],x_des[-1])

    # 生成泛化轨迹
    x_rollout, _, _ = dmp.rollout(x0=[17,3,3],xT=[16.2,2.5,3])
    x_rollout2, _, _ = dmp.rollout_ECDMP(x0=[17,3,3],xT=[16.2,2.5,3])

    
    np.savetxt(r'D:\大论文\实验代码\Joing_dmps\numerical_test\case3\x_des', x_des)
    np.savetxt(r'D:\大论文\实验代码\Joing_dmps\numerical_test\case3\dmp', x_rollout)
    np.savetxt(r'D:\大论文\实验代码\Joing_dmps\numerical_test\case3\ec-dmp', x_rollout2)
