...
class CartesianDMP():
    ...
    def __init__(self,N_bf=300,alphaz=1,betaz=1.0,orientation=False):
...
if __name__ == "__main__":
    ...
    data = np.loadtxt(r'D:\大论文\实验代码\experiment_tpe\dataset\embeddings1.csv')
    ...
    demo1 = np.zeros((1000,3))
    from scipy import interpolate
    time = 1
    timestamps = np.linspace(0,1,len(demo[:,0]),endpoint=True)
    f_interpolate = interpolate.interp1d(timestamps,demo[:,0])
    time_new5 = np.linspace(0, time, 1000)
    demo1[:,0] = f_interpolate(time_new5)

    time = 1
    timestamps = np.linspace(0,1,len(demo[:,1]),endpoint=True)
    f_interpolate = interpolate.interp1d(timestamps,demo[:,1])
    time_new5 = np.linspace(0, time, 1000)
    demo1[:,1] = f_interpolate(time_new5)

    plt.title('t0')
    plt.plot( demo1[:,0] ,demo1[:,1])
    plt.show()



    dmp = CartesianDMP(alphaz=0,betaz=25/4.0,N_bf=10,orientation=False)
    x_des = dmp.imitate(demo1)
    x_rollout, dx_rollout, ddx_rollout = dmp.rollout()

    dmp2 = CartesianDMP(alphaz=1.0,betaz=20/4.0,N_bf=200,orientation=False)
    x_des = dmp2.imitate(demo1)
    ...
    plt.plot(x_rollout[:,0],x_rollout[:,1],color=colors[1],label=f"DMP1",lw=2,linestyle='--')
    ...
    dp_x1_rollout = f_interpolate(time_new5)

    time = 1
    timestamps = np.linspace(0,1,len(x_rollout[:,1]),endpoint=True)
    f_interpolate = interpolate.interp1d(timestamps,x_rollout[:,1])
    time_new5 = np.linspace(0, time, 300)
    dp_y1_rollout = f_interpolate(time_new5)



    # time = 1
    # timestamps = np.linspace(0,1,len(data2[:,0]),endpoint=True)
    # f_interpolate = interpolate.interp1d(timestamps,data1[:,0])
    # time_new5 = np.linspace(0, time, 200)
    # x_true = f_interpolate(time_new5)

    time = 1
    timestamps = np.linspace(0,1,len(x_rollout2[:,1]),endpoint=True)
    f_interpolate = interpolate.interp1d(timestamps,x_rollout2[:,1])
    time_new5 = np.linspace(0, time, 300)
    dp_y2_rollout = f_interpolate(time_new5)



    plt.title('t5')
    plt.plot(dp_x1_rollout,dp_y1_rollout)
    plt.plot(dp_x1_rollout,dp_y2_rollout)
    plt.show()
    ...
    line_DMP1, = plt.plot([], [], marker='.',linestyle='--', markersize=5, color='red', label='DMP', )
    line_DMP2, = plt.plot([], [], marker='*',linestyle='--', markersize=4, color='b', label='TPE-DMP')
...
def update(frame):
    line_DMP1.set_data(dp_x1_rollout[:frame], dp_y1_rollout[:frame])
    # line_DMP2.set_data(dp_x1_rollout[:frame],dp_y2_rollout[:frame])
...
# 创建动画对象
ani = FuncAnimation(fig, update, frames=len(x_rollout[:,1]), init_func=init, blit=True)
# plt.axis('off') 

plt.title(f"Position between demon", fontsize=10)
...
ani.save('animation_gt3.gif', writer=PillowWriter(fps=120),dpi=300)
